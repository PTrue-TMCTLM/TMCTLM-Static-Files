...
// VERSIÓN MODIFICADA POR ARDORA: www.webardora.net

include_once ("Gestion_de_usuarios_para.php");
...
if (!isset ($slogin_path)) { $slogin_path = "";}
$slogin_userstxt = $slogin_path . $slogin_userstxt;
$slogin_altuser = $slogin_path . $slogin_altuser;

if (!isset ($slogin_lang)) {$slogin_lang = "";}
if (!isset ($slogin_noauthpage)) {$slogin_noauthpage = 0;}
if (!isset ($slogin_redirect)) {$slogin_redirect = "";}
if (!isset ($slogin_logoutredirect)) {$slogin_logoutredirect = "";}
$slogin_lang = "es";
...
if (!$slogin_php_self) {$slogin_php_self = $HTTP_SERVER_VARS["PHP_SELF"];
  if (!$slogin_php_self) {$slogin_php_self = $_ENV["PHP_SELF"];
    if (!$slogin_php_self) {$slogin_php_self = getenv("PHP_SELF");}
...
if ($_GET["logout"]) {$slogin_logout = 1; } 
	else { $slogin_logout = 0; 
}

if ($_POST["slogin_POST_explicitauth"]) { $slogin_explicitauth = 1; } else { $slogin_explicitauth = 0; }

if ($_POST["slogin_POST_username"]) { 
   $slogin_loginname = substr (trim (preg_replace("/[^[:alnum:]_.@]/", "", $_POST["slogin_POST_username"])), 0, SLOGIN_MAXLEN_USERNAME);} 
else { $slogin_loginname = ""; 
}

if ($_POST["slogin_POST_password"]) { 
   $slogin_loginpass = substr (trim (preg_replace("/[^[:alnum:]_.@]/", "",$_POST["slogin_POST_password"])), 0, SLOGIN_MAXLEN_PASSWORD); } 
else { $slogin_loginpass = "";
}

if ($_SESSION["Username"]) { $slogin_Username = substr ($_SESSION["Username"], 0, SLOGIN_MAXLEN_USERNAME); } 
	else { $slogin_Username = ""; 
}
...
if ($slogin_logout) {
   ...
   include_once ($slogin_path . "../Gestion_de_usuarios/" .$arquiPass);
   exit;
}
else{
       ...
       if ((!$slogin_Username) && (!$slogin_Password)) {
          if ((!$slogin_loginname) && (!$slogin_loginpass)) {
           ...
           if (fslogin_check_user ($slogin_loginname, $slogin_loginpass)) {
              ...
              if ($slogin_redirect) {
                 if (strtoupper ($slogin_loginname) != SLOGIN_ADMIN_USERNAME) {
                    if ($slogin_allowuser_redirect) {header ("Location: " . $slogin_redirect . $slogin_Username);}
                    else {header ("Location: " . $slogin_redirect);}
                    exit;
                 }
                 else{//header ("Location: adminlog.php"); //rediriximos á pantalla de administración;
                 }
               }
       }
    else {
...
if (SLOGIN_USERS_FILE) {
   ...
   if (trim ($user) == trim ($username)) {
      ...
      if (trim ($slogin_pass[$key]) == trim ($password)) {
         $slogin_content[0] = $username;
         $slogin_content[1] = $password;
         return ($slogin_content);
       }
...
global $slogin_user,  $slogin_pass, $slogin_name , $slogin_curs, $slogin_grup, $slogin_profe;
  foreach ($slogin_user as $key => $user) {
     if (strtoupper (trim ($user)) == strtoupper (trim ($username))) {
        if (strtoupper (trim ($tipo)) == strtoupper (trim ("NOM"))) {return ($slogin_name[$key]);}
        if (strtoupper (trim ($tipo)) == strtoupper (trim ("CUR"))) {return ($slogin_curs[$key]);}
        if (strtoupper (trim ($tipo)) == strtoupper (trim ("GRU"))) {return ($slogin_grup[$key]);}
        if (strtoupper (trim ($tipo)) == strtoupper (trim ("TIP"))) {return ($slogin_profe[$key]);}
      }
   }
}
...
global $slogin_php_self;
    $logs_dirhandle = opendir ("."); $logs_sorteddir = array (); $logs_count = 0;
    while (($logs_file = readdir ($logs_dirhandle)) != false)
          $logs_sorteddir[count ($logs_sorteddir)] = $logs_file;
          closedir ($logs_dirhandle); rsort ($logs_sorteddir); echo "<ul>\n";
...
global $slogin_php_self, $slogin_lang, $slogin_text;
     $baks_dirhandle = opendir ("."); $baks_sorteddir = array (); $baks_count = 0;
...
function fslogin_norm_poststring ($poststr, $maxlen = 0) {
     ...
     if (get_magic_quotes_gpc ()) { $poststr = stripslashes ($poststr);}
     if ($maxlen > 0) { $poststr = substr ($poststr, 0, ($maxlen - 1));}
     return ($poststr);
}
